create database fazendaPalmito;
use fazendapalmito;
...
INSERT INTO palmito (tipo_palmito, preco_venda, estoque_atual, data_plantio, data_colheita)
VALUES
...
INSERT INTO fornecedor (nome_fornecedor, contato, cidade)
VALUES
...
INSERT INTO venda (id_palmito, quantidade_vendida, data_venda, preco_total)
VALUES 
...
select * from venda where id_palmito = 1;
...
select palmito.tipo_palmito "Tipo palmito" , count(*) as "Quantidade de vendas" from venda join palmito using (id_palmito) group by id_palmito;

#d. Consulta 4: Listar os tipos de palmito cuja venda total excedeu 500 unidades, utilizando HAVING.
select palmito.tipo_palmito "Tipo palmito", venda.quantidade_vendida "Quantidade vendida" from venda join palmito using(id_palmito) having quantidade_vendida > 500;
...
begin
declare qtd int;
select sum(quantidade_vendida) into qtd from venda where id_palmito = id;
return ifnull(qtd, 0);
end //
delimiter ;
...
delimiter //
create function calculaValorTotal(id int, qtd int) returns decimal(10,2)
begin
declare valorTotal decimal(10,2);
declare valorUn decimal(10,2);
select palmito.preco_venda into valorUn from palmito where palmito.id_palmito = id;
set valorTotal = valorUn * qtd;
...
create procedure registraVenda(in id_palmito int, in qtd int)
begin
declare valorTotal decimal(10,2);
...
create procedure atualizaEstoque (in id_palmito int, in qtd int)
begin
declare estoqueNovo int;
set estoqueNovo = calculaEstoque (id_palmito, qtd);
update palmito set estoque_atual = estoqueNovo where palmito.id_palmito = id_palmito;
end //

create trigger vender after insert on venda
for each row
begin
	call atualizaEstoque(new.id_palmito, new.quantidade_vendida);
end //

delimiter ;
call registraVenda(1, 10);
select * from venda where id_palmito =1;
